#!/usr/bin/env node
/**
 * Cutover Desayunos POS → Winston Servicios en Vercel (servicios-admin).
 * Lee INSFORGE_DESAYUNOS_* desde .env.local (no imprime secrets).
 */
...
const NAMES = ['INSFORGE_DESAYUNOS_URL', 'INSFORGE_DESAYUNOS_API_KEY']
...
if (!String(envLocal.INSFORGE_DESAYUNOS_URL).includes('g4ta4bfg')) {
  console.error('✗ INSFORGE_DESAYUNOS_URL no apunta a Winston Servicios (g4ta4bfg)')
  process.exit(1)
}

if (!fs.existsSync(path.join(ROOT, '.vercel/project.json'))) {
  run('npx -y vercel link --yes --project servicios-admin')
