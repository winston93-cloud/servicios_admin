...
export const ESTATUS_ALUMNO_OPCIONES = [
  ...
  { valor: 5, etiqueta: 'Bloqueo psicológico / académico' },
] as const

/** Estatus 4 y 5: no avanzan de ciclo (bloqueo psicología o académico). */
export const ESTATUS_ALUMNO_BLOQUEO = 4
export const ESTATUS_ALUMNO_BLOQUEO_PSICO = 5
export const ESTATUS_ALUMNO_BLOQUEOS = [4, 5] as const

export function esEstatusBloqueo(status: number | null | undefined): boolean {
...
export function claseTagEstatusAlumno(status: number | null): string {
  switch (status) {
    ...
    case ESTATUS_ALUMNO_BLOQUEO_PSICO:
