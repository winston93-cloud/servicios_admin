...
export async function esDeudorReinscrito(
  ...
  if (mes === 2 && cea !== 17) {
    const materialFeb = alumnoTienePagoSemiref(pagos, ref, '16', cea)
    if (!materialFeb) return true
  }

  if (mes > 5) {
    const cicloJunio = mmddHoy() < cambioCicloMmDd() ? cea : cea - 1
    const junioPagado = alumnoTienePagoSemiref(pagos, ref, '10', cicloJunio)
