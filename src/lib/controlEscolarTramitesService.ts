...
import {
  esFolioPagoInternoEnTalon,
  reconciliarTramitesPendientesConPagos,
} from '@/lib/controlEscolarTramitesSync'

export {
  cancelarTramitesPorPagoIds,
  reconciliarTramitesPendientesConPagos,
  sincronizarTramiteConPagoInterno,
  sincronizarTramitesPorPagoIds,
} from '@/lib/controlEscolarTramitesSync'
export type { AccionSyncTramiteCe, MotivoCancelacionTramiteCe } from '@/lib/controlEscolarTramitesSync'
...
export async function listarTramitesAdministrativos(): Promise<{
  ...
  const db = createDbAdmin()
  try {
    await reconciliarTramitesPendientesConPagos(db)
  } catch (e) {
    console.error('reconciliarTramitesPendientesConPagos:', e)
  }
  ...
  const folioVivoPorPago = new Map<number, number>()
  for (let i = 0; i < pagoIds.length; i += 200) {
    const chunk = pagoIds.slice(i, i + 200)
    if (!chunk.length) continue
    const { data: pagosVivos } = await db
      .from('pago_interno')
      .select('pago_id, pago_folio, pago_cancelado')
      .in('pago_id', chunk)
    for (const p of pagosVivos ?? []) {
      const pid = Number(p.pago_id)
      if (Number(p.pago_cancelado) === 0 && esFolioPagoInternoEnTalon(Number(p.pago_folio))) {
        folioVivoPorPago.set(pid, Number(p.pago_folio))
      }
    }
  }
  ...
  for (const row of data ?? []) {
    const pagoId = Number(row.pago_id)
    const folioVivo = folioVivoPorPago.get(pagoId)
    const rowConFolio =
      folioVivo != null && folioVivo !== Number(row.pago_folio)
        ? { ...row, pago_folio: folioVivo }
        : row
    const fila = filaDesdeRow(
      rowConFolio as never,
      alumnos.get(Number(row.alumno_id)),
      conceptoOtroPorPago.get(pagoId) ?? null
    )
