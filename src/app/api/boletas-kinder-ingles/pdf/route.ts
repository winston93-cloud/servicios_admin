...
import { generarPdfKinderEn } from '@/lib/boletasKinderEnService'
...
export async function GET(req: Request) {
  try {
    ...
    const buf = await generarPdfKinderEn({ alumnoId, bimestre, ciclo })
    return new Response(new Uint8Array(buf), {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="boleta_kinder_en_${alumnoId}_t${bimestre}_${ciclo}.pdf"`,
