...
import { generarPdfKinderEs } from '@/lib/boletasKinderEsService'
...
export async function GET(req: Request) {
  try {
    ...
    const buf = await generarPdfKinderEs({ alumnoId, bimestre, ciclo })
    return new Response(new Uint8Array(buf), {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `inline; filename="boleta_kinder_es_${alumnoId}_t${bimestre}_${ciclo}.pdf"`,
